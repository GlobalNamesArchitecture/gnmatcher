...
public class Position implements Comparable<Position> {
    ...
    private static enum EditDistanceRelationType {
        AT_MAX, NOT_AT_ZERO_AND_NOT_AT_MAX, AT_ZERO_AND_NOT_AT_MAX
    }
    ...
    private static enum PositionType {
        STANDARD_POSITION, TRANSPOSITION_POSITION
    }
    ...
    private static enum RelevantSubwordHitIndexType {
        FIRST_INDEX, SECOND_INDEX, TRAILING_INDEX, NO_INDEX
    }
        ...
        switch (edRelationType) {
            case AT_ZERO_AND_NOT_AT_MAX:            //we are allowed to execute edit operations (none have been conducted yet)
            {
                switch (sRSSizeType) {
                    ...
                    default:
                        return INSERTION_TRANSITION;    //there are no more boundaries in the automaton's String
                }
            }
            ...
            default:                //we are NOT allowed to execute more edit operations 
            {
                switch (sRSSizeType) {
                    case ZERO:
                        return FAILURE_TRANSITION;   //there are no more boundaries in the automaton's String
    ...
    public State transitionInternal(int maxEditDistance, int relevantSubwordSize, int hitIndex) {
        ...
        switch (hitIndex) {
            case -1:
                rsHitIndexType = Position.RelevantSubwordHitIndexType.NO_INDEX;
                break;
            case 0:
                rsHitIndexType = Position.RelevantSubwordHitIndexType.FIRST_INDEX;
                break;
            case 1:
                rsHitIndexType = Position.RelevantSubwordHitIndexType.SECOND_INDEX;
                break;
            default:
                rsHitIndexType = Position.RelevantSubwordHitIndexType.TRAILING_INDEX;
                break;
        }
        /////
